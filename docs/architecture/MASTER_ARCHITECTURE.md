...
graph TD
    ...
    style CourtFlow fill:#082f49,stroke:#38bdf8,stroke-width:2px,color:#fff
...
2. **🔵 Electric Sky Blue (`#38bdf8` / `#0284c7`) with Mint (`#10b981`) - The "Active Play" Domain**
   * *Purpose*: Live court queue rotation, player standing check-ins, and active session play.
   * *Active in*: CourtFlow App & Landing Page (`#38bdf8` / `#0284c7`), Leagues Player Portal (`#10b981`), PWA Queue Wrappers.
...
### 📜 ADR-01: Video Walkthroughs Over Live Demo Portals
*   **Stance:** Deprecate live public demo/sandbox portals (`?c=demo`). Instead, pair interactive UI mockups on marketing pages with curated video screen recordings.
*   **Rationale:** Maintaining a perpetual public demo database requires continuous data resetting, state curation, and backend synchronization across two products. Screen recordings deliver a deterministic, zero-friction product tour while keeping production and staging environments strictly isolated.
*   **Execution:** Remove public demo host mappings from the club registry and PWA shells, and feature role-specific screen recording walkthroughs directly below the interactive UI previews on marketing pages.
